...
# Check if two addresses match on Address Line 1 and 2 (number + street)
...
def process_files(forms_df, ups_df):
    ...
    # Clean UPS relevant address columns
    for col in ["AC_Name", "Attention_Name", "Address Line 1", "Address Line 2", "City", "Address Line 3", "Address Type", "Postal_Code", "Country_Code", "Address_Country_Code"]:
        if col in ups_df.columns:
            ups_df[col] = ups_df[col].astype(str).apply(remove_tones).str.strip()

    # Clean Forms relevant address columns (only those we use)
    forms_address_cols = [col for col in forms_df.columns if isinstance(col, str)]
    for col in forms_address_cols:
        forms_df[col] = forms_df[col].astype(str).apply(remove_tones).str.strip()

    # Group UPS data by account number normalized
    ...
    for idx, form_row in forms_df.iterrows():
        ...
        if is_same_billing == "yes":
            ...
            # Validate address number and street (address line 1 and 2) exist in UPS addresses
            ...
            # Upload template rows (01, 06, 03 with invoice options 1, 2, 6)
            ...
            # Billing/delivery and pickups separate processing (max 3 pickups)
            ...
            # Add pickups to upload template
            for pu_addr in pickup_addrs:
                upload_template_rows.append({
                    "AC_NUM": form_row["Account Number"],
                    "AC_Address_Type": "02",
            ...
            # Add billing addresses (01, 06, 03 with invoice options)
            upload_template_rows.append({
                "AC_NUM": form_row["Account Number"],
                "AC_Address_Type": "01",
                "invoice option": "",
    ...
    # Add unmatched Forms responses that were not processed
    ...
    for col in cols_order:
        if col not in upload_template_df.columns:
            upload_template_df[col] = ""
...
def main():
    ...
    st.write("Upload Microsoft Forms response and UPS system address files to validate and generate upload template.")
    ...
    if forms_file and ups_file:
        with st.spinner("Processing..."):
            ...
            if not unmatched_df.empty:
                ...
                st.download_button(
                    label="Download Unmatched Excel",
            ...
            if not upload_template_df.empty:
                ...
                st.dataframe(upload_template_df)
