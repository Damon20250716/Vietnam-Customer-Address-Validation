        ...
        if is_same_billing == "yes":
            ...
            for _, ups_row in ups_acc_df.iterrows():
                if ups_row["Address Type"] == '01':  # address type 01 match only
                    if flexible_address_match(new_addr1, ups_row["Address Line 1"], threshold=0.5) and \
                       flexible_address_match(new_addr2, ups_row["Address Line 2"], threshold=0.5):
                        matched_in_ups = True
            ...
            def check_address_in_ups(addr1, addr2, addr_type_code):
                for _, ups_row in ups_acc_df.iterrows():
                    if ups_row["Address Type"] == addr_type_code:
                        if flexible_address_match(addr1, ups_row["Address Line 1"], threshold=0.5) and \
                           flexible_address_match(addr2, ups_row["Address Line 2"], threshold=0.5):
                            return ups_row
                return None
